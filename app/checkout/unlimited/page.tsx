  ...
  useEffect(() => {
    async function loadUser() {
      try {
        ...
        console.error(
          "Failed to load saved user for Unlimited checkout:",
          error
        );
      } finally {
  ...
  async function handleStartCheckout() {
    try {
      ...
      if (savedUser.plan === "unlimited") {
        alert("This account is already on Unlimited.");
        return;
      }
        ...
        <section className="rounded-[2rem] border border-[#c1123f]/10 bg-white/72 p-6 shadow-[0_20px_60px_rgba(111,0,23,0.05)] sm:p-8 lg:p-10">
          <div className="space-y-4">
            ...
            <p className="max-w-2xl text-base leading-8 text-black/65 sm:text-lg">
              Continue to Stripe to unlock the full companion experience with
              unlimited saved messages, unlimited personal selfies, and
          ...
          <div className="mt-8 rounded-[1.75rem] border border-[#c1123f]/8 bg-[#fff8f8] p-5 sm:p-6">
            <div className="space-y-4 text-sm text-black/68 sm:text-base">
              ...
              <p className="flex items-center justify-between gap-3">
                <span>Price</span>
                <span className="font-semibold text-black">£29.99 / month</span>
              </p>
          ...
          {isAlreadyUnlimited ? (
            <div className="mt-5 rounded-[1.5rem] border border-amber-200 bg-amber-50 px-5 py-4 text-sm leading-7 text-amber-900">
              This account is already on Unlimited, so starting Unlimited
              checkout is blocked to avoid accidental duplicate subscriptions.
            </div>
          ) : null}
